...
<div class="swal_popup_BlocklyUserExampleViewer_bottom_left_div">
  ...
  <div id="userExamplesButton_menu2_shareLinkToExample">
    <i class="fa-solid fa-share-from-square"></i> &#8226 Share link to Example
  </div>
</div>`,
    customClass: {
...
export default {
  ...
  methods: {
    ...
    async userexamples() {
      await getSessionID();
      let SERVER = 'https://s4d-examples.fsh.plus/';
      if (window.isInS4DDebugMode) {
        SERVER = prompt('Examples server URL to use?', SERVER);
  ...
  <div class="box">
    <input type="radio" onchange="window.examplesSelectStatus(false)" id="${example.id}" name="pickThisExampleToImportButton" class="sr-only-basic">
      <center>
        <h4>${example.name}</h4>
...
<button class="swal2-confirm swal2-styled" id="upload-btn">Upload Example</button>`;
            Swal.fire({
              theme: document.querySelector('[data-bs-theme="light"]') ? 'light' : 'dark',
...
${blockCounts <= 5 ? `<p style="color: red; font-weight: bold;">Uploading near empty examples is not encouraged.</p>` : ''}`,
            ...
            function search() {
              ...
              mutation = mutation.filter((example) => {
                if (!caseSensitive) {
                  example.name = example.name.toLowerCase();
                  example.description = example.description.toLowerCase();
                  example.creator = example.creator.toLowerCase();
                }
                return example.name.includes(query) || example.description.includes(query) || example.creator.includes(query) || example.id.includes(query);
              });
