...
export default {
  ...
  mounted() {
    ...
    if (String(window.location.pathname).replace(/\//gim, '') == 'spooky') {
      ...
      ['emptyObject_inVent.png', 'emptyObject.gif', 'glitch1.png', 'glitch2.png', 'gottenThread_inVent.png', 'gottenThread.gif', 'history.gif', 'night2.png', 'office_door_left.png', 'office_door_right.png', 'office_shine.png', 'office_unlit.png', 'power.png', 'powerMeter_back.png', 'powerMeter_filled.png', 'static.gif'].forEach((img) => {
  ...
  <ol style="text-align:left">
    ...
    <li>Your bot is started!</li>
        ...
        if (json) {
          ...
          let requires = [`"discord.js": "^14.26.2",`, `"process":"^0.11.10",`, `"easy-json-database": "^1.5.0",`, `"discord-logs": "^2.2.1",`];
      ...
      if (String(window.location.pathname).replace(/\//gim, '') == 'spooky') {
        ...
        if (random != 0) {
          ...
          const powerDecreaseInterval = setInterval(() => {
            ...
            powerLevel -= 1;
            if (office.doors.left.closed) powerLevel -= 1;
            GAMESCORE++;
            if (office.doors.right.closed) powerLevel -= 1;
            GAMESCORE++;
          ...
          function playJumpscare(character) {
            ...
            setTimeout(() => {
              ...
              setTimeout(() => {
                ...
                setTimeout(() => {
                  // i LOVE call backs!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                  scoring.innerHTML += ' ' + String(GAMESCORE).replace(/</gim, '&lt;');
...
${window.aprilFoolsTheme ? '<button id="btn-aprilend" class="swal2-deny swal2-styled">End Aprill Fools</button>' : ''}`,
        ...
        didOpen: () => {
          ...
          if (document.getElementById('btn-aprilend'))
...
${
  ...
  <button data-id="${prebuild}" data-act="del" class="swal2-deny swal2-styled p-2 m-0">Delete</button>
    ...
    .join('<hr class="m-1">') || '<span>No prebuilds, create one!</span>'
                    ...
                    cancelButtonText: 'Cancel',
                    confirmButtonText: 'Save Prebuild',
                    didOpen: () => {
                      Array.from(document.querySelectorAll('.swal2-html-container button.swal2-styled')).forEach((btn) => {
                        btn.onclick = async () => {
                          ...
                          if (action === 'del') {
                            ...
                            Toast.fire({
                              theme: 'auto',
                              icon: 'success',
                              title: this.$t('prebuild.deletee.success')
                            });
                          } else if (action === 'down') {
                            ...
                            zip.generateAsync({ type: 'blob' }).then((blob) => {
                              ...
                              const url = window.URL.createObjectURL(blob);
                              a.href = url;
                              a.download = `${id.replace('prebuild-', '')}.s4d`;
                              a.click();
                              window.URL.revokeObjectURL(url);
                              document.body.removeChild(a);
                            });
                            ...
                            Toast.fire({
                              theme: 'auto',
                              icon: 'success',
                              title: this.$t('prebuild.download.successs')
                            });
                          } else if (action === 'load') {
                            const token = await localforage.getItem(id);
                            ...
                            Toast.fire({
                              theme: 'auto',
                              icon: 'success',
                              title: this.$t('prebuild.successs')
                            });
                          }
                        };
                      });
...
<center>
  ...
  <b>Switching themes may require a refresh to work properly.</b>
    ...
    console() {
      ...
      try {
        fetch('https://469runtest.jeremygamer13.repl.co/console')
