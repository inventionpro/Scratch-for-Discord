export default `<xml xmlns="https://developers.google.com/blockly/xml">
  <variables>
    <variable>arguments</variable>
    <variable>command</variable>
  </variables>
  <block type="s4d_login" x="259" y="199">
    <value name="TOKEN">
      <shadow type="text">
        <field name="TEXT">Your bot token</field>
      </shadow>
    </value>
  </block>
  <block type="jg_event_message_when_a_message_is_recieved_and_author_isnt_a_bot" x="262" y="284">
    <statement name="STATEMENTS">
      <block type="jg_comments_connected_comment">
        <field name="TEXT">We can use the shortcut block to avoid&amp;#10;having to check the author first</field>
        <next>
          <block type="jg_comments_connected_comment">
            <field name="TEXT">(these are comment blocks btw so that things can&amp;#10;be explained better)</field>
            <next>
              <block type="variables_set">
                <field name="VAR">arguments</field>
                <value name="VALUE">
                  <block type="lists_split">
                    <mutation mode="SPLIT"></mutation>
                    <field name="MODE">SPLIT</field>
                    <value name="INPUT">
                      <block type="s4d_message_content"></block>
                    </value>
                    <value name="DELIM">
                      <shadow type="text">
                        <field name="TEXT"> </field>
                      </shadow>
                    </value>
                  </block>
                </value>
                <next>
                  <block type="variables_set">
                    <field name="VAR">command</field>
                    <value name="VALUE">
                      <block type="lists_getIndex">
                        <mutation statement="false" at="true"></mutation>
                        <field name="MODE">GET_REMOVE</field>
                        <field name="WHERE">FROM_START</field>
                        <value name="VALUE">
                          <block type="variables_get">
                            <field name="VAR">arguments</field>
                          </block>
                        </value>
                        <value name="AT">
                          <block type="math_number">
                            <field name="NUM">1</field>
                          </block>
                        </value>
                      </block>
                    </value>
                    <next>
                      <block type="controls_if">
                        <value name="IF0">
                          <block type="logic_compare">
                            <field name="OP">EQ</field>
                            <value name="A">
                              <block type="variables_get">
                                <field name="VAR">command</field>
                              </block>
                            </value>
                            <value name="B">
                              <block type="text">
                                <field name="TEXT">!post</field>
                              </block>
                            </value>
                          </block>
                        </value>
                        <statement name="DO0">
                          <block type="jg_comments_connected_comment">
                            <field name="TEXT">This uses a command handler to make things easier.</field>
                            <next>
                              <block type="s4d_reply">
                                <value name="CONTENT">
                                  <shadow type="text">
                                    <field name="TEXT">You have written the following content after !say:</field>
                                  </shadow>
                                  <block type="text_join">
                                    <mutation items="3"></mutation>
                                    <value name="ADD0">
                                      <block type="text">
                                        <field name="TEXT">Posting \`\`</field>
                                      </block>
                                    </value>
                                    <value name="ADD1">
                                      <block type="lists_split">
                                        <mutation mode="JOIN"></mutation>
                                        <field name="MODE">JOIN</field>
                                        <value name="INPUT">
                                          <block type="variables_get">
                                            <field name="VAR">arguments</field>
                                          </block>
                                        </value>
                                        <value name="DELIM">
                                          <shadow type="text">
                                            <field name="TEXT"> </field>
                                          </shadow>
                                        </value>
                                      </block>
                                    </value>
                                    <value name="ADD2">
                                      <block type="text">
                                        <field name="TEXT">\`\` to site...</field>
                                      </block>
                                    </value>
                                  </block>
                                </value>
                                <next>
                                  <block type="jg_comments_connected_comment">
                                    <field name="TEXT">The URL is the API/Site you want to interact with.&amp;#10;The API in this example is a fake one, but it's&amp;#10;most likely going to be similar to how you need&amp;#10;to do things.</field>
                                    <next>
                                      <block type="jg_web_request_advanced_new_send_request">
                                        <field name="METHOD">"post"</field>
                                        <value name="URL">
                                          <block type="text">
                                            <field name="TEXT">https://www.turtsocielmediaaLoL.me/api/post</field>
                                          </block>
                                        </value>
                                        <statement name="HEADERS">
                                          <block type="jg_web_request_advanced_header">
                                            <value name="KEY">
                                              <block type="text">
                                                <field name="TEXT">Authorization</field>
                                              </block>
                                            </value>
                                            <value name="VALUE">
                                              <block type="frost_env">
                                                <value name="VALUE">
                                                  <shadow type="text">
                                                    <field name="TEXT">api-key</field>
                                                  </shadow>
                                                </value>
                                              </block>
                                            </value>
                                          </block>
                                        </statement>
                                        <statement name="BODY">
                                          <block type="jg_web_request_advanced_data_section">
                                            <field name="LABEL">post</field>
                                            <statement name="STATEMENTS">
                                              <block type="jg_web_request_advanced_data">
                                                <value name="KEY">
                                                  <block type="text">
                                                    <field name="TEXT">content</field>
                                                  </block>
                                                </value>
                                                <value name="VALUE">
                                                  <block type="lists_split">
                                                    <mutation mode="JOIN"></mutation>
                                                    <field name="MODE">JOIN</field>
                                                    <value name="INPUT">
                                                      <block type="variables_get">
                                                        <field name="VAR">arguments</field>
                                                      </block>
                                                    </value>
                                                    <value name="DELIM">
                                                      <shadow type="text">
                                                        <field name="TEXT"> </field>
                                                      </shadow>
                                                    </value>
                                                  </block>
                                                </value>
                                              </block>
                                            </statement>
                                          </block>
                                        </statement>
                                        <statement name="THEN">
                                          <block type="jg_comments_connected_comment">
                                            <field name="TEXT">The response codes 200 or below should mean&amp;#10;we made the request successfully!&amp;#10;You can learn what these response codes are here:&amp;#10;https://developer.mozilla.org/en-US/docs/Web/HTTP/Status</field>
                                            <next>
                                              <block type="controls_if">
                                                <mutation else="1"></mutation>
                                                <value name="IF0">
                                                  <block type="logic_compare">
                                                    <field name="OP">LTE</field>
                                                    <value name="A">
                                                      <block type="jg_web_request_advanced_response_status"></block>
                                                    </value>
                                                    <value name="B">
                                                      <block type="math_number">
                                                        <field name="NUM">200</field>
                                                      </block>
                                                    </value>
                                                  </block>
                                                </value>
                                                <statement name="DO0">
                                                  <block type="s4d_reply">
                                                    <value name="CONTENT">
                                                      <shadow type="text">
                                                        <field name="TEXT">The post was sent!</field>
                                                      </shadow>
                                                    </value>
                                                  </block>
                                                </statement>
                                                <statement name="ELSE">
                                                  <block type="jg_comments_connected_comment">
                                                    <field name="TEXT">Something may have gone wrong.&amp;#10;Lets tell that to the user.&amp;#10;In this case, the API also sends back the error in&amp;#10;a key called "error", so we can send that back&amp;#10;too.</field>
                                                    <next>
                                                      <block type="s4d_reply">
                                                        <value name="CONTENT">
                                                          <shadow type="text">
                                                            <field name="TEXT">Hey!</field>
                                                          </shadow>
                                                          <block type="text_join">
                                                            <mutation items="5"></mutation>
                                                            <value name="ADD0">
                                                              <block type="text_multiline">
                                                                <field name="TEXT">Oops! Something may have gone wrong!&amp;#10;The site responded with status **</field>
                                                              </block>
                                                            </value>
                                                            <value name="ADD1">
                                                              <block type="jg_web_request_advanced_response_status"></block>
                                                            </value>
                                                            <value name="ADD2">
                                                              <block type="text_multiline">
                                                                <field name="TEXT">**.&amp;#10;\`\`</field>
                                                              </block>
                                                            </value>
                                                            <value name="ADD3">
                                                              <block type="jg_web_request_advanced_get_response_data">
                                                                <value name="VALUE">
                                                                  <block type="text">
                                                                    <field name="TEXT">error</field>
                                                                  </block>
                                                                </value>
                                                              </block>
                                                            </value>
                                                            <value name="ADD4">
                                                              <block type="text">
                                                                <field name="TEXT">\`\`</field>
                                                              </block>
                                                            </value>
                                                          </block>
                                                        </value>
                                                      </block>
                                                    </next>
                                                  </block>
                                                </statement>
                                              </block>
                                            </next>
                                          </block>
                                        </statement>
                                        <statement name="IF_ERROR">
                                          <block type="jg_comments_connected_comment">
                                            <field name="TEXT">We got a server error, lets show it in console.</field>
                                            <next>
                                              <block type="s4d_print">
                                                <value name="PRINT">
                                                  <block type="frost_other_err"></block>
                                                </value>
                                              </block>
                                            </next>
                                          </block>
                                        </statement>
                                      </block>
                                    </next>
                                  </block>
                                </next>
                              </block>
                            </next>
                          </block>
                        </statement>
                      </block>
                    </next>
                  </block>
                </next>
              </block>
            </next>
          </block>
        </next>
      </block>
    </statement>
  </block>
  <block type="jg_unused_floating_comment" x="1033" y="1009">
    <field name="TEXT">This API here wants us to be a real user.&amp;#10;Lets login with what it asks for, an API key.&amp;#10;The API you are using should tell you how to get&amp;#10;this API key. This is usually going to be in&amp;#10;the header, and should be stored in something&amp;#10;like a process.env block to keep it safe.</field>
  </block>
  <block type="jg_comments_floating_image" x="1720" y="1002">
    <field name="TEXT">/builtin_examples/advjsonreq1.png</field>
  </block>
  <block type="jg_comments_floating_arrow" x="952" y="1111">
    <field name="arrow">left</field>
  </block>
  <block type="jg_unused_floating_comment" x="1257" y="1206">
    <field name="TEXT">The "post" data section is what this API wants.&amp;#10;A request object in S4D should be data section, so&amp;#10;a requests body would be a data section named&amp;#10;"body".&amp;#10;"content" is what we are going to post on their&amp;#10;site.</field>
  </block>
  <block type="jg_comments_floating_arrow" x="1176" y="1254">
    <field name="arrow">left</field>
  </block>
  <block type="jg_comments_floating_image" x="1044" y="1702">
    <field name="TEXT">/builtin_examples/advjsonreq2.png</field>
  </block>
</xml>`;
